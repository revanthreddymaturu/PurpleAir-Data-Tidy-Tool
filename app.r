...
server <- function(input, output, session) {
    ...
    Raw_data <- reactive({
        ...
        Rawdf <- Rawdf %>%
            mutate(
                pm2_5_atm = ifelse(pm2_5_atm >= 500, NA, pm2_5_atm),
    ...
    Corr_data <- reactive({
        ...
        if (input$channelset == TRUE) {
            Purple_Data <- Purple_Data %>%
                mutate(AB_dif_per1 = (abs(PM1_A - PM1_B) / ((PM1_A + PM1_B) / 2))) %>% # Calculates percent diff
                ...
                mutate(PM_Check10 = ifelse(is.na(PM10_A) | is.na(PM10_B), coalesce(PM10_A, PM10_B), PM10_AVG))
            ...
            Purple_Data <- Purple_Data %>%
                ...
                mutate(AB_dif_val10 = abs(PM10_A - PM10_B)) %>% # Checks difference between Channels
                mutate(PM10_AVG = ((PM10_A + PM10_B) / 2)) %>% # Calculates AVG between A & B Channels
                mutate(PM_Check10 = if_else(AB_dif_per10 <= 0.7 | AB_dif_val10 <= 5, PM10_AVG, NA_real_))
        ...
        if (input$smokeset == TRUE) {
            ...
            corr_data <- corr_data %>% mutate(PM2.5_corr_EPA = case_when(
                is.numeric(PM2.5) ~ 0.524 * PM2.5 - 0.0862 * RH + 5.75,
                TRUE ~ NA_real_
            ))
        } else {
            # Other Method using EPA Multi Point Correction for High PM2.5 (smoke) *Newer Equation (Dec 2022)*
            corr_data <- corr_data %>% mutate(PM2.5_corr_EPA = case_when(
                (0 <= PM2.5) & (PM2.5 < 570) ~ PM2.5 * 0.524 - 0.0862 * RH + 5.75,
                (570 <= PM2.5) & (PM2.5 < 611) ~ ((0.0244 * PM2.5 - 13.9) * (((PM2.5)^{
                    2
                }) * (4.21 * 10^{
                    -4
                }) + (PM2.5 * 0.392) + 3.44) + (1 - (0.0244 * PM2.5 - 13.9)) * (PM2.5 * 0.524 - 0.0862 * RH + 5.75)),
                (611 <= PM2.5) ~ ((PM2.5)^{
                    2
                }) * (4.21 * 10^{
                    -4
        ...
        corr_data <- select(corr_data, "date", "PM1", "PM2.5", "PM2.5_corr_EPA", "PM10", "RH", "Temperature", "Pressure") # Reorder df
    ...
    Daily_data <- reactive({
        ...
        columns_to_aggregate <- c("PM1", "PM2.5", "PM2.5_corr_EPA", "PM10", "RH", "Temperature", "Pressure")
    ...
    output$dailyfile <- DT::renderDataTable({
        ...
        tryCatch(
            {
                dailyDF <- Daily_data()
                validate(need(ncol(dailyDF) == 10, "Dataframe is missing columns please review data for errors/corruption."))
                # Corrdf
                Tbloption <- Zone_sel()
